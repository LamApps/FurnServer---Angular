...
export class AdminMenu {
  constructor(
	...
	getMenu(company: any): Observable<NbMenuItem[]> {
		if(company.id==0) {
			...
			const params = JSON.stringify({ cid: company.id }) 
			const menus = [
				{
					title: 'Users',
					icon: 'person-outline',
			...
			menus.forEach(menu=>{
				const permissionMenu = this.getPermissionMenu(company, menu);
				if(permissionMenu) companysMenu.push(permissionMenu);
			})
			return new Observable(observer => {
